...
1. Manually upload the source to `modules/allsecureexchange`.
1. Go to `Modules` > `Module Catalog` in your shop's admin area.
1. Select category `Other`.
1. Click `Install` on the `AllSecure Exchange` item.
...
1. Click on `Configure` on the `AllSecure Exchange` item.
